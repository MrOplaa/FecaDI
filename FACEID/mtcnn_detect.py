...
class MTCNNDetect(object):
    def __init__(self, face_rec_graph, model_path = "models", threshold = [0.6, 0.7, 0.7], factor = 0.709, scale_factor = 1):
        ...
        self.factor = factor # đươc sử dụng để tạo kim tự tháp -- phát hiện mặt
    ...
    def detect_face(self, img, minsize): # trả về bouding box & points
        # Phát hiện khuôn mặt trong ảnh (img), trả về boudingbox và các point
        ...
        points = []
        # lấy chiều dài và rộng của tấm ảnh
        h = img.shape[0]
        w = img.shape[1]
        minl = np.amin([h, w])
        m = 12.0 / minsize
        ...
        #print(simple_points)
        return rects, simple_points * self.scale_factor # rect = (a,b,c,d), [points]
